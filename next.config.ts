...
const nextConfig: NextConfig = {
  ...
  async headers() {
    ...
    // VS Codes Mobile Preview rendert die lokale App in einem iframe.
    // In Produktion bleibt Framing weiterhin vollständig gesperrt.
    ...
    // (Marketing, Login, Root). Diese bleiben statisch/CDN-cachebar. Die
    // authentifizierten App-Routen (/berichte, /baustellen, /einstellungen,
    ...
    return [
      ...
      ...(process.env.NODE_ENV === "production"
        ? [
            {
              source: "/((?!berichte|baustellen|einstellungen|api).*)",
              headers: [
                { key: "Content-Security-Policy", value: lenientCsp },
              ],
            },
          ]
        : []),
